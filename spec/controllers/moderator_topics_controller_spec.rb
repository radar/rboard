...
describe Moderator::TopicsController do
  ...
  fixtures :topics, :users, :forums, :moderations, :group_users, :groups, :permissions, :posts
  before do
    @admin_topic = topics(:admin)
    @moderator_topic = topics(:moderator)
    @user_topic = topics(:user)
    @moderation = moderations(:second)
    @other_moderation = moderations(:third)
  end
  ...
  describe "as moderator" do
    ...
    describe "when ids are passed in as params" do
      ...
      it "should be able to move a set of topics" do
        post 'moderate', { :moderation_ids => [@moderation.id], :new_forum_id => forums(:everybody), :commit => "Move" }
      ...
      it "should be able to merge two topics" do
        put 'merge', { :master_topic_id => @moderator_topic.id, :new_subject => "New Subject" }, { :user => users(:moderator).id, :moderation_ids => [@moderator_topic.id, @user_topic.id] }
        flash[:notice].should eql(t(:topics_merged))
        response.should redirect_to(forums_path)
      end
      
      it "should not be able to merge a topic from the admin forum" do
        put 'merge', { :master_topic_id => @moderator_topic.id, :new_subject => "New Subject" }, { :user => users(:moderator).id, :moderation_ids => [@moderator_topic.id, @admin_topic.id] }
        flash[:notice].should eql(t(:topics_not_accessible_by_you))
        response.should redirect_to(forums_path)
      end
      
      it "should not be able to merge topics that do not exist" do
        put 'merge', { :master_topic_id => @moderator_topic.id, :new_subject => "New Subject" }, { :user => users(:moderator).id, :moderation_ids => [4,12,87] }
